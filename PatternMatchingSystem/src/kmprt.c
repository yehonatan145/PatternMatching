...
int* kmp_create_failure_table(char* pattern, int n) {
    int *failure_table = (int*) malloc ((n + 1) * sizeof(int));
...
int kmp_get_period(char* pattern, int n) {
    int* failure_table = kmp_create_failure_table(pattern, n);
    int ret =  n - failure_table[n];
    free(failure_table);
    return ret;
}
...
int _kmp_read_char(KMPRealTime* kmp, char c) {
    if (kmp->pattern[kmp->offset] == c) {
        ...
        //printf("keep looping, offset is %d, flag is %d\n", kmp->offset, kmp->flag);
        // If got here, there is need in looping through the failure function
        kmp->flag |= KMP_LOOP_FAIL_FLAG;
        /*
...
int kmp_read_char(KMPRealTime* kmp, char c) {
    int i;
    if (kmp->flag & KMP_LOOP_FAIL_FLAG) { // Have failure function moves to do
        //printf("backtracing - offset: %d, flag: %d\n", kmp->offset, kmp->flag);
        ...
        return 0;
    } else if (kmp->flag & KMP_HAVE_BUFFER_FLAG) { // Have chars waiting in buffer
        //printf("have buffer of length %d - offset: %d, flag: %d\n", kmp->buf_end - kmp->buf_start + 1, kmp->offset, kmp->flag);
        _kmp_add_char_to_buffer(kmp, c);
        for (i = 0; i < 2; ++i) {
        ...
        //printf("after working on buffer, offset: %d, flag: %d\n", kmp->offset, kmp->flag);
        return 0;
    } else {
